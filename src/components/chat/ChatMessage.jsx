...
export default function ChatMessage({ message, isUser, timestamp }) {
  return (
    <motion.div
      ...
      className={`flex gap-2 md:gap-3 mb-3 md:mb-4 ${isUser ? "flex-row-reverse" : "flex-row"}`}
    >
      {/* Avatar */}
      <div
      ...
      <div className={`flex flex-col max-w-[85%] md:max-w-[70%] ${isUser ? "items-end" : "items-start"}`}>
        <div
          className={`rounded-2xl px-3 py-2 md:px-4 md:py-3 shadow-md ${
            isUser
              ? "bg-gradient-to-br from-blue-500 to-indigo-600 text-white rounded-tr-sm"
              : "bg-white text-gray-800 rounded-tl-sm border border-gray-200"
          }`}
        >
          <div className="text-sm md:text-base whitespace-pre-wrap break-words prose prose-sm md:prose-base max-w-none [&>*:first-child]:mt-0 [&>*:last-child]:mb-0">
            <ReactMarkdown
              components={{
                h1: ({node, ...props}) => (
                  <h1
                ...
                ul: ({node, ...props}) => (
                  <ul
                    className={`${isUser ? 'text-white' : 'text-gray-800'} list-disc pl-5 my-1 space-y-0`}
                    {...props}
                  />
                ),
                ol: ({node, ...props}) => (
                  <ol
                    className={`${isUser ? 'text-white' : 'text-gray-800'} list-decimal pl-5 my-1 space-y-0`}
                    {...props}
                  />
                ),
                li: ({node, children, ...props}) => (
                  <li
                    className={`${isUser ? 'text-white' : 'text-gray-800'} leading-snug my-0 py-0 [&>p]:inline [&>p]:m-0 [&>p]:p-0`}
